                ...
                <v-flex xs12>
                  ...
                  >
                    <template
                      slot="selection"
                      slot-scope="data"
                    >{{ data.item.Departamento}}, {{data.item.Municipio}}</template>

                    <template slot="item" slot-scope="data">
                      <v-list-tile-content>
                        <v-list-tile-title
                          v-html="`${data.item.Departamento}, ${data.item.Municipio}`"
                        ></v-list-tile-title>
                      </v-list-tile-content>
                    </template>
                  </v-select>
                  ...
                  <div style="float:left; width:50%">
                    <v-flex xs12 sm6 md4>
                      ...
                      >
                        ...
                        <v-date-picker
                          v-model="Fecha_de_nacimiento"
                          @input="menu_nacimiento = false, computeAge(Fecha_de_nacimiento)"
                          color="#3A4750"
                          locale="es-ES"
                        ></v-date-picker>
                      </v-menu>
                  ...
                  <div>
                    <v-text-field v-model="Edad" label="Edad" :disabled="true" outline></v-text-field>
                  </div>
                  <v-text-field
        ...
        <v-card>
          <v-card-title primary-title>
            <v-flex xs12>
              ...
              <v-container fluid>
                <v-checkbox
                  v-model="Sindrome_Clinico_Presentacion"
                  label="Sx. Nefrítico"
                  color="green"
                  :value="1"
                ></v-checkbox>
                ...
                <v-checkbox
                  v-model="Sindrome_Clinico_Presentacion"
                  label="Anomalías Urinarias Asintomáticas"
                  color="green"
                  :value="3"
                ></v-checkbox>
                <v-checkbox
        ...
        <v-card>
          <v-card-title primary-title>
            <v-flex xs12>
              ...
              <v-textarea outline v-model="Historia" name="Historia" rows="40" :rules="nombreRules"></v-textarea>
            </v-flex>
          </v-card-title>
        </v-card>
      </div>
...
export default {
  mounted() {
    this.$http
      .get(
        `http://${process.env.SERVER_IP}:8000/PacienteController/findOne?CUI=${this.$route.params.cui}`
      )
      .then((response) => {
    ...
    this.$http
      .get(
        `http://${process.env.SERVER_IP}:8000/ProcedenciaController/getAllLocation`
      )
      .then((response) => {
  ...
  data() {
    return {
      ...
      cuiRules: [
        ...
        (v) =>
          (v && v.length <= 13) || "Verifique que el CUI sea de 13 digitos.",
      ],
      nombreRules: [
        (v) => !!v || "Se requiere este campo",
      ...
      phoneRules: [
        ...
        (v) =>
          (v && v.length <= 8) ||
          "Verifique que teléfono tenga como máximo 8 dígitos.",
        (v) =>
          (v && this.isANumber(v)) || "Verifique dato ingresado sean números.",
      ],
  ...
  methods: {
    save() {
      ...
      if (
        info.Edad.substring(info.Edad.length - 5, info.Edad.length) === "meses"
      ) {
        info.Edad =
          parseFloat(this.Edad.substring(0, info.Edad.length - 6)) / 12.0;
      } else {
      ...
      this.$http
        .put(
          `http://${process.env.SERVER_IP}:8000/PacienteController/updateAll`,
          info
        )
        .then((response) => {
          this.error = false;
          ...
          this.infoMessage =
            "Los datos del paciente fueron actualizados exitosamente.";
        ...
        .catch((error) => {
          ...
          console.log("Error");
          this.infoMessage =
            "Ocurrió un error al actualizar los datos del paciente.";
          this.infoDialog = true;
        });
    },
    ...
    checkLocation(inStr) {
      ...
      this.$http
        .post(
          `http://${process.env.SERVER_IP}:8000/ProcedenciaController/locate/`,
          data
        )
        .then((response) => {

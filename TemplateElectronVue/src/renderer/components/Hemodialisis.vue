  ...
  <div>
    ...
    <v-card>
      <v-toolbar flat color="primary" dark>
        ...
        >Pacientes agendados para hoy: {{todaysDateFormatted}}</v-toolbar-title>
      </v-toolbar>
      <v-tabs centered fixed-tabs vertical dark v-model="activeTab">
        ...
        >{{paciente.Nombre}} {{paciente.Apellido}}</v-tab>
        <v-tab-item
          v-for="paciente in this.pacientes"
          :key="paciente.nombre"
          :title="paciente.Nombre"
            ...
            <form class="form-horizontal">
              <fieldset>
                <div class="encapsulado">
                  ...
                  <div class="DatosPaciente">
                    ...
                    <div v-if="paciente.Imagen">
                      <img
                        id="fotoPaciente"
                        style="margin-left: auto; margin-right: auto; width:50% margin-top: 2%"
                        :src="paciente.Imagen"
                        alt
                        width="273"
                        height="183"
                    ...
                    <div v-else>
                      <!-- se utiliza la imagen default si no se ha escogido una  -->
                      <img
                        id="fotoPaciente"
                        style="margin-left: auto; margin-right: auto; width:50% margin-top: 2%"
                    ...
                    <!-- demas datos -->
                    <div>
                      <v-text-field :value="paciente.Nombre" label="Nombre" filled disabled></v-text-field>
                      <v-text-field :value="paciente.Apellido" label="Apellido" filled disabled></v-text-field>
                      <v-text-field :value="paciente.CUI" label="CUI" filled disabled></v-text-field>
                      <v-text-field
                    ...
                    <div>
                      <v-text-field :value="paciente.Edad" label="Edad" filled disabled></v-text-field>
                      <v-text-field
                  ...
                  <v-layout align-center justify-end style="width: 100%">
                    <v-container fluid row align-center justify-space-around fill-height>
                      ...
                      <v-checkbox
                        ...
                        v-model="tabs[pacientes.indexOf(paciente)].Procedencia"
                        label="Hospitalizado"
                        value="1"
                        color="green"
                      ></v-checkbox>
                  ...
                  <ol>
                    <ul>
                      ...
                      <v-layout align-center justify-end>
                        <v-container fluid row align-center justify-space-around fill-height>
                          <v-checkbox
                            class="align-center justify-center"
                            v-model="tabs[pacientes.indexOf(paciente)].Hemodialisis"
                            label="Aguda"
                            value="1"
                            color="green"
                          ></v-checkbox>
                    ...
                    <ul id="via">
                      ...
                      <v-container row>
                        <v-checkbox
                          class="align-center justify-center"
                          v-model="tabs[pacientes.indexOf(paciente)].Via.Lugar"
                          label="Femoral"
                          value="Femoral"
                          color="green"
                        ></v-checkbox>
                      ...
                      <v-container row>
                        <v-checkbox
                          class="align-center justify-center"
                          v-model="tabs[pacientes.indexOf(paciente)].Via.Direccion"
                          label="Invertido"
                          value="Invertido"
                          color="green"
                        ></v-checkbox>
                    ...
                    <ul>
                      ...
                      <v-container row align-center justify-space-around fill-height>
                        <v-checkbox
                          class="align-center justify-center"
                          v-model="tabs[pacientes.indexOf(paciente)].Conductividad.Na"
                          label="Na+"
                          color="green"
                        ></v-checkbox>
                        <v-checkbox
                          class="align-center justify-center"
                          v-model="tabs[pacientes.indexOf(paciente)].Conductividad.K"
                          label="K+"
                          color="green"
                        ></v-checkbox>
                        <v-checkbox
                          class="align-center justify-center"
                          v-model="tabs[pacientes.indexOf(paciente)].Conductividad.HCO3"
                          label="HCO3"
                          color="green"
                        ></v-checkbox>
                      </v-container>
                ...
                <button
                  ...
                  v-on:click="activateTableDialog"
                >Comenzar hemodiálisis</button>
                <!-- cuadro de dialogo de aviso para el autosave de tabla -->
                <v-dialog
                  v-model="tabs[pacientes.indexOf(paciente)].tableDialog"
                  persistent
                  max-width="40%"
                >
                  <v-card class="dialogText">
                    ...
                    <v-card-text>
                      ...
                      <br />
                      <br />En caso que desee volver a revisar la tabla, haga clic en "Revisar", este aviso volverá a aparecer en 5 minutos.
                      <br />
                      <br />¿Es toda la información ingresada correcta?
                    </v-card-text>
                    <v-card-actions>
                      <v-spacer></v-spacer>
                      <v-btn
                ...
                <button
                  ...
                  class="btn btn-lg btn-warning btn-block"
                  v-on:click="guardar"
                  :disabled="!tabs[pacientes.indexOf(paciente)].pass"
...
export default {
  mounted() {
    ...
    const month =
      this.todaysDate.getUTCMonth() + 1 < 10
        ? "0" + (this.todaysDate.getUTCMonth() + 1)
        : this.todaysDate.getUTCMonth() + 1;
    const fechahoy =
    ...
    this.$http
      .get(
        `http://${process.env.SERVER_IP}:8000/PacienteController/findWithAppointment?fecha=${fechahoy}`
      )
      .then((response) => {
        ...
        for (let index = 0; index < response.data.Pacientes.length; index++) {
          ...
          if (edadPaciente < 1) {
            response.data.Pacientes[index].Edad =
              Math.round(edadPaciente * 12.0) + " meses";
          } else {
        ...
        for (let index = 0; index < response.data.Pacientes.length; index++) {
          ...
          // this.$http.get(`http://${process.env.SERVER_IP}:8000/hemodialisis/getHemoData?id=${pacienteid}?NoHemodialisis=${nohemo}`).then(response => {
          this.$http
            .get(
              `http://${process.env.SERVER_IP}:8000/hemodialisis/getHemoData?id=${pacienteid}&NoHemodialisis=${nohemo}`
            )
            .then((response) => {
              ...
              if (consulta.length !== 0) {
                current_tab["Hemodialisis"] = String(
                  consulta[0].TipoDeHemodialisis
                );
                current_tab["Via"] = JSON.parse(consulta[0].Via);
                current_tab["Lineas_pediatricas"] =
                  consulta[0].LineasPediatrica;
                ...
                current_tab["Heparinizacion"] = JSON.parse(
                  String(consulta[0].Heparinizacion)
                );
                current_tab["Tiempo"] = JSON.parse(consulta[0].Tiempo);
                current_tab["Conductividad"] = JSON.parse(
                  consulta[0].Conductividad
                );
                ...
                current_tab["Procedencia"] = String(
                  consulta[0].LugarDeProcedencia
                );
            ...
            .catch((error) => {
              this.tabs[index].infoMessage =
                "Ocurrió un error al obtener los datos de esta cita.";
  ...
  data() {
    return {
      ...
      expedienteRules: [
        ...
        (v) =>
          (v && this.isANumber(v)) ||
          "Verifique que el dato ingresado sea un número.",
      ],
      nombreRules: [
        (v) => !!v || "Se requiere este campo",
  ...
  methods: {
    gettodaysDateFormatted() {
      ...
      this.todaysDateFormatted = "".concat(
        daysOfWeek[engDateDay],
        ", ",
        engDateDate,
        " de ",
        months[engDateMonth],
    ...
    guardar() {
      ...
      this.$http
        .post(`http://${process.env.SERVER_IP}:8000/hemodialisis`, info)
        .then((response) => {
          console.log(response);
          this.tabs[this.activeTab].infoMessage =
            "La información ingresada sobre la cita de hemodiálisis ha sido guardada exitosamente.";
          this.tabs[this.activeTab].infoDialog = true;
        })
        .catch((error) => {
          this.tabs[this.activeTab].infoMessage =
            "Ocurrió un error al ingresar los datos de la cita.";
          this.tabs[this.activeTab].infoDialog = true;
        });
    },
    ...
    saveTableData() {
      ...
      this.$http
        .post(
          `http://${process.env.SERVER_IP}:8000/hemodialisis/storeTable`,
          info
        )
        .then((response) => {
          ...
          for (let i = 0; i < 9; i++) {
            ...
            td.style =
              " text-align: left; max-width: 10%; border: 3px solid #a9a9a9;";
            td.contentEditable = true;
            tr.append(td);
          }
          table.append(tr);
          if (table.rows.length - 1 < 8) {
            //el formulario dice que se tienen hasta 8 entradas en la tabla
            var loop = setInterval(() => {
              this.activateTableDialog();
              clearInterval(loop);
            }, 900 * 1000); //15min

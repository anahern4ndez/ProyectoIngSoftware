              ...
              <v-card-text>
                <v-flex xs12 sm6 md4>
                  <div>
                    ...
                    >
                      <option
                        v-for="item in estados_response"
                        :key="item.ID"
                        :value="item.ID"
                      >{{item.significado}}</option>
                    </select>
                  </div>
                </v-flex>
              </v-card-text>
            ...
            <template slot="items" slot-scope="props">
              <tr @click="changeSelected(props.item)">
                ...
                <td class="justify-center layout px-0">
                  <v-icon small class="ma-3" @click="editarDatos(props.item)">edit</v-icon>
                  <v-icon small class="ma-3" @click="showDeleteDiag()">delete</v-icon>
                </td>
              </tr>
            </template>
          ...
          <form>
            ...
            <div>
              ...
              <input
                type="file"
                class="hide_file"
                style="height:auto; width:auto; visibility:hidden"
            ...
            <div style="float:left; width: 50%">
              ...
              <h3
                class="subheading font-weight-light"
                style="padding-left:10%"
              >{{selectedPatients.EstadoActual}}</h3>
            </div>
            <div style="float:left; width: 50%">
            ...
            <div style="float:left; width: 45%">
              ...
              <h3
                class="subheading font-weight-light"
                style="padding-left:10%"
              >{{selectedPatients.Nombre_de_padre}}</h3>
            </div>
            <div style="float:left; width: 50%; margin-left:5%">
              ...
              <h3
                class="subheading font-weight-light"
                style="padding-left:10%"
              >{{selectedPatients.Nombre_de_madre}}</h3>
            </div>
            <br />
            <br />
          </form>
    ...
    <div id="botones">
      ...
      <div class="boton">
        <button
          type="button"
          class="btn btn-lg btn-warning btn-block"
          v-on:click="ingresarNuevo"
          v-if="$can('crear pacientes')"
        >Ingresar nuevo paciente</button>
      </div>
      <div class="boton" v-if="$can('dar consulta')">
        <router-link
          :to="{name: 'gestionarPaciente', params: { idPaciente: this.selectedPatients.id, cui: this.selectedPatients.CUI }}"
          class="btn btn-lg btn-warning btn-block"
        >Dar Consulta</router-link>
      </div>
      <div class="boton">
        <router-link
          :to="{name: 'cambioEstado', params: { cui: this.selectedPatients.CUI, nombre: this.selectedPatients.Nombre, edad: this.selectedPatients.Edad, sexo: this.selectedPatients.Sexo, estado: this.selectedPatients.estado_actual, registro: this.selectedPatients.Numero_expediente }}"
          class="btn btn-lg btn-warning btn-block"
        >Editar estado de paciente seleccionado</router-link>
      </div>
      <br />
      <div class="boton" style="width: 94%;" v-if="$can('ingresar formularios')">
        <router-link
          :to="{name: 'gestionFormularios', params: { cui: this.selectedPatients.CUI, nombre: this.selectedPatients.Nombre, apellido: this.selectedPatients.Apellido }}"
          class="btn btn-lg btn-warning btn-block"
        >Ingresar formulario de este paciente</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeSelected(received) {
      ...
      this.selectedPatients.EstadoActual = this.pacientes[
        this.pacientes.indexOf(received)
      ].estado_actual.significado;
    ...
    editarDatos(received) {
      ...
      this.$router.push({
        name: "EditarPaciente",
        params: { cui: this.editedItem.CUI },
      });
    },
    close() {
      this.dialog = false;
    ...
    deleteItem(item) {
      ...
      this.$http
        .delete(
          `http://${process.env.SERVER_IP}:8000/PacienteController/delete?cui=${this.deletedCUI}`
        )
        .then((response) => {
          this.reloadTable();
          this.close();
